...
ACMD(do_say)
{
  ...
  else
  {
    ...
    snprintf(buf, sizeof(buf), "$n\tn says, '%s%s%s'", CCCYN(ch, C_NRM),argument, CCNRM(ch, C_NRM));
    ...
    {
      sprintf(buf, "You say, %s'%s'%s", CCCYN(ch, C_NRM),argument,CCNRM(ch, C_NRM));
...
static int is_tell_ok(struct char_data *ch, struct char_data *vict)
{
  ...
  else if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_SOUNDPROOF) && (GET_REAL_LEVEL(ch) < LVL_GOD))
    send_to_char(ch, "The walls seem to absorb your words.\r\n");
  else if (!IS_NPC(vict) && !vict->desc)        /* linkless */
    act("$E's linkless at the moment.", FALSE, ch, 0, vict, TO_CHAR | TO_SLEEP);
  else if (PLR_FLAGGED(vict, PLR_WRITING))
    act("$E's writing a message right now; try again later.", FALSE, ch, 0, vict, TO_CHAR | TO_SLEEP);
  else if ((!IS_NPC(vict) && PRF_FLAGGED(vict, PRF_NOTELL)) || (ROOM_FLAGGED(IN_ROOM(vict), ROOM_SOUNDPROOF) && (GET_REAL_LEVEL(ch) < LVL_GOD)))
...
ACMD(do_tell)
{
  ...
  if (!*buf || !*buf2)
    ...
    *q = '\0';
	...
	else if (GET_REAL_LEVEL(ch) < LVL_IMMORT && !(vict = get_player_vis(ch, buf, NULL, FIND_CHAR_WORLD)))
    send_to_char(ch, "%s", CONFIG_NOPERSON);
  else if (GET_REAL_LEVEL(ch) >= LVL_IMMORT && !(vict = get_char_vis(ch, buf, NULL, FIND_CHAR_WORLD)))
    send_to_char(ch, "%s", CONFIG_NOPERSON);
...
ACMD(do_gen_comm)
{
  ...
  if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_SOUNDPROOF) && (GET_REAL_LEVEL(ch) < LVL_GOD)) {
    send_to_char(ch, "The walls seem to absorb your words.\r\n");
    return;
  }
  ...
  if (GET_REAL_LEVEL(ch) < CONFIG_LEVEL_CAN_SHOUT) {
  ...
  for (i = descriptor_list; i; i = i->next) {
    ...
    if (ROOM_FLAGGED(IN_ROOM(i->character), ROOM_SOUNDPROOF) && (GET_REAL_LEVEL(ch) < LVL_GOD))
      continue;
