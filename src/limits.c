...
int mana_gain(struct char_data *ch)
{
  ...
  {
	  if (GET_MANA(ch) >= GET_MAX_MANA(ch)) return (0);
...
int hit_gain(struct char_data *ch)
{
  ...
  {
    if (GET_HIT(ch) >= GET_MAX_HIT(ch)) return (0);
...
int move_gain(struct char_data *ch)
{
  ...
  {
		if (GET_MOVE(ch) >= GET_MAX_MOVE(ch)) return (0);
...
void set_title(struct char_data *ch, char *title)
{
  ...
  if (title == NULL) {
    GET_TITLE(ch) = strdup(GET_SEX(ch) == SEX_FEMALE ?
      title_female(GET_CLASS(ch), GET_REAL_LEVEL(ch)) :
      title_male(GET_CLASS(ch), GET_REAL_LEVEL(ch)));
  } else {
