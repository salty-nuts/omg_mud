...
cpp_extern const struct command_info cmd_info[] = {
  ...
  { "affects"  , "aff"     , POS_DEAD    , do_affect   , 0, 0 },
  { "afk"      , "afk"     , POS_DEAD    , do_gen_tog  , 0, SCMD_AFK },
  { "areas"    , "are"     , POS_DEAD    , do_areas    , 0, 0 },
	
  { "assault"   , "assa"   , POS_STANDING, do_assault   , 1, 0 },
  ...
  { "autoexitl", "autoexitl" , POS_DEAD    , do_gen_tog , 0, SCMD_AUTOEXIT_L },
  ...
  { "circle"   , "circ"    , POS_FIGHTING, do_circle   , 1, 0 },
  { "claim"    , "claim"   , POS_STANDING, do_claim    , 0, 0 },
//  { "climb"    , "climb"   , POS_STANDING, do_climb    , 0, 0 },
  ...
  { "happyhour", "ha"      , POS_DEAD    , do_happyhour, 0, 0 },
	{ "headbutt" , "head"		 , POS_FIGHTING, do_headbutt, 1,0},
  ...
  { "metagame" , "meta"    , POS_RESTING , do_metagame , 1, 0 },
  ...
  { "train"    , "trai"    , POS_RESTING , do_train    , 1, 0 },
  ...
  { "taste"    , "tas"     , POS_RESTING , do_eat      , 0, SCMD_TASTE },
	{ "taunt"		 , "taun"    , POS_FIGHTING, do_taunt		 , 1, 0 },
  ...
  { "trigedit" , "trigedit", POS_DEAD    , do_oasis_trigedit, LVL_BUILDER, 0 },
	{ "tumble"	 , "tum"		 , POS_FIGHTING, do_tumble	 , 0, 0 },
...
long is_long_int(const char *str)
{
  if(*str == '-')
     str++;
  if(!*str)
...
int enter_player_game (struct descriptor_data *d)
{
   ...
   * Salty
   * 09 FEB 2020
   * Change loadroom to recall if character has been gone too long
   *
*/
...
*/
  ...
  case CON_QCLASS:
    ...
    write_to_output(d,"\n\rYour abilities are:\n\r");
    write_to_output(d,"STR: %d, INT: %d, WIS: %d, DEX: %d, CON: %d, LUCK: %d", GET_STR(d->character),
			GET_INT(d->character), GET_WIS(d->character), GET_DEX(d->character), GET_CON(d->character),
			GET_LUCK(d->character));
