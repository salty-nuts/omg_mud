...
int do_simple_move(struct char_data *ch, int dir, int need_specials_check)
{
  ...
  if (ROOM_FLAGGED(going_to, ROOM_NOMOB) && IS_NPC(ch))
    return (0);
...
int perform_move(struct char_data *ch, int dir, int need_specials_check)
{
  ...
  bool pass_door = FALSE;

  if (ch == NULL || dir < 0 || dir >= NUM_OF_DIRS || FIGHTING(ch))
    return (0);

  /*
   *  Salty's Confusing Zones
   *  01 FEB 2020
   */
  if (ZONE_FLAGGED(GET_ROOM_ZONE(IN_ROOM(ch)), ZONE_CONFUSING)) {
   dir = rand_number(0, DIR_COUNT - 1); /* Select a random diredction */
  }

  if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_CONFUSING)) {
   dir = rand_number(0, DIR_COUNT - 1); /* Select a random diredction */
  }
  ...
  else if (EXIT_FLAGGED(EXIT(ch, dir), EX_CLOSED) )
  {
    ...
    {
      if ( AFF_FLAGGED(ch, AFF_PASS_DOOR) || AFF_FLAGGED(ch, AFF_IMP_PASS_DOOR))
      pass_door = TRUE;
    }
...
static int ok_pick(struct char_data *ch, obj_vnum keynum, int pickproof, int scmd)
{
  ...
  else if (percent > skill_lvl)
  {
    ...
    check_improve(ch,SKILL_PICK_LOCK,FALSE);
  }
  else
  {
    check_improve(ch,SKILL_PICK_LOCK,TRUE);
    return (1);
  }
...
ACMD(do_follow)
{
  ...
  if (*buf) {
    if (!(leader = get_char_vis(ch, buf, NULL, FIND_CHAR_WORLD))) {
      send_to_char(ch, "%s", CONFIG_NOPERSON);
      return;
    }
  } else {
