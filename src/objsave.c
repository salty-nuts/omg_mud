    ...
    char *arg, int mode)
{
  ...
  if (CMD_IS("rent")) {
    ...
    if (mode == RENT_FACTOR) {
      ...
      mudlog(NRM, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE, "%s has rented (%d/day, %ld tot.)",
...
static int Crash_load_objs(struct char_data *ch) {
  ...
  if (rentcode == RENT_RENTED || rentcode == RENT_TIMEDOUT)
  {
    ...
    {
      GET_BANK_GOLD(ch) -= LMAX(cost - GET_GOLD(ch), 0);
      GET_GOLD(ch) = LMAX(GET_GOLD(ch) - cost, 0);
      save_char(ch);
    }
  }
