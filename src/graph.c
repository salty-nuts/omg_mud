...
ACMD(do_track)
{
  ...
  if (rand_number(0, 101) >= GET_SKILL(ch, SKILL_TRACK))
  {
    ...
    check_improve(ch,SKILL_TRACK,FALSE);
    return;
  }
  /* They passed the skill check. */
  TRACK: dir = find_first_step(IN_ROOM(ch), IN_ROOM(vict));
	int roll, skill;
	roll = rand_number(0,101);
	skill = GET_SKILL(ch, SKILL_HUNT);
     switch (dir)
     {
     case BFS_ERROR:
     ...
     default:	/* Success! */
		   ...
		   if (skill > roll)
    	 {
				 send_to_char(ch,"You are hunting down %s\n\r",GET_NAME(vict));
      	 perform_move(ch, dir, 1);
      	 check_improve(ch, SKILL_HUNT, TRUE);
      	 goto TRACK;
    	 }
    	 else
			 {
      	 check_improve(ch, SKILL_HUNT, FALSE);
				break;
			 }
       break;
     }
     check_improve(ch,SKILL_TRACK,TRUE);
