...
static void perform_mob_name_list(struct char_data * ch, char *buf);

static void perform_mob_name_list(struct char_data * ch, char *arg)
{
  ...
  len = snprintf(buf, sizeof(buf), "Listing mobiles with %s%s%s name.\r\n", QYEL, arg, QNRM);

  for (num = 0; num <= top_of_mobt; num++)
  {

    if (isname(arg, mob_proto[num].player.name))
    {
...
static void perform_mob_level_list(struct char_data * ch, char *arg)
{
  ...
  if (mob_level < 0 || mob_level > LVL_IMPL)
  {
...
ACMD(do_oasis_list)
{
  ...
  switch (subcmd) {
	...
	send_to_char(ch, "       %smlist flags <num>%s   - List all mobiles with flag set\r\n", QYEL, QNRM);
        send_to_char(ch, "       %smlist name <arg>%s    - List all mobiles with keyword <name>\r\n", QYEL, QNRM);
	send_to_char(ch, "Just type %smlist flags%s to view available options.\r\n", QYEL, QNRM);
        return;
      }
      else if (is_abbrev(arg, "name") || is_abbrev(arg, "keyword"))
      {
	if (!*arg2)
	{
	  send_to_char(ch, "Which mobile keyword do you want to list?\r\n");
	  return;
	}
	else
	 perform_mob_name_list(ch,arg2);
      }
