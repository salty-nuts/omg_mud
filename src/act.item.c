...
static void perform_give_gold(struct char_data *ch, struct char_data *vict, long amount);
...
ACMD(do_put)
{
  ...
  if (!*theobj)
    ...
    generic_find(thecont, FIND_OBJ_EQUIP | FIND_OBJ_INV | FIND_OBJ_ROOM, ch, &tmp_char, &cont);
    if (!cont)
      send_to_char(ch, "You don't see %s %s here.\r\n", AN(thecont), thecont);
    else if (GET_OBJ_TYPE(cont) != ITEM_CONTAINER)
      act("$p is not a container.", FALSE, ch, cont, 0, TO_CHAR);
    else if (OBJVAL_FLAGGED(cont, CONT_CLOSED) && (GET_REAL_LEVEL(ch) < LVL_IMMORT || !PRF_FLAGGED(ch, PRF_NOHASSLE)))
...
static void get_check_money(struct char_data *ch, struct obj_data *obj)
{
  ...
  if (GROUP(ch) && (value > 0) && PRF_FLAGGED(ch, PRF_AUTOSPLIT))
  {
    sprintf(buf,"%d",value);
    do_split(ch, buf, 0, 0);
			     ...
			     char *arg, int mode, int howmany)
{
  ...
  if (OBJVAL_FLAGGED(cont, CONT_CLOSED) && (GET_REAL_LEVEL(ch) < LVL_IMMORT || !PRF_FLAGGED(ch, PRF_NOHASSLE)))
		       ...
		       struct obj_data *obj)
{
  ...
  if (IS_CARRYING_N(vict) >= CAN_CARRY_N(vict) && GET_REAL_LEVEL(ch) < LVL_IMMORT && GET_REAL_LEVEL(vict) < LVL_IMMORT) {
    act("$N seems to have $S hands full.", FALSE, ch, 0, vict, TO_CHAR);
    return;
  }
  if (GET_OBJ_WEIGHT(obj) + IS_CARRYING_W(vict) > CAN_CARRY_W(vict) && GET_REAL_LEVEL(ch) < LVL_IMMORT && GET_REAL_LEVEL(vict) < LVL_IMMORT) {
...
static void perform_give_gold(struct char_data *ch, struct char_data *vict, long amount)
{
  ...
  if ((GET_GOLD(ch) < amount) && (IS_NPC(ch) || (GET_REAL_LEVEL(ch) < LVL_GOD))) {
    send_to_char(ch, "You don't have that many coins!\r\n");
    return;
  }
  send_to_char(ch, "%s", CONFIG_OK);

  snprintf(buf, sizeof(buf), "$n gives you %ld gold coin%s.", amount, amount == 1 ? "" : "s");
  act(buf, FALSE, ch, 0, vict, TO_VICT);

  snprintf(buf, sizeof(buf), "$n gives %s to $N.", money_desc(amount));
  act(buf, TRUE, ch, 0, vict, TO_NOTVICT);

  if (IS_NPC(ch) || (GET_REAL_LEVEL(ch) < LVL_GOD))
    decrease_gold(ch, amount);
...
ACMD(do_drink)
{
  ...
  if (GET_OBJ_VAL(temp, 3) && GET_REAL_LEVEL(ch) < LVL_IMMORT) { /* The crap was poisoned ! */
...
ACMD(do_eat)
{
  ...
  if ((GET_OBJ_TYPE(food) != ITEM_FOOD) && (GET_REAL_LEVEL(ch) < LVL_IMMORT))
  {
    /*
       Quest tokens can be eaten by everyone!
       Salty
       13 NOV 2019
     */
    if (GET_OBJ_TYPE(food) == ITEM_TOKEN)
    {
      send_to_char(ch,"You feel slightly odd eating this, but it is satisfying.\n\r");
      send_to_char(ch,"There were %d quest points contained within that token.\n\r",GET_OBJ_VAL(food,0));
      GET_QUESTPOINTS(ch) += GET_OBJ_VAL(food, 0);
      extract_obj(food);
      return;
    }
  ...
  if (GET_OBJ_VAL(food, 3) && (GET_REAL_LEVEL(ch) < LVL_IMMORT)) {
...
static void perform_wear(struct char_data *ch, struct obj_data *obj, int where)
{
  ...
  int wear_bitvectors[] = {
    ...
    ITEM_WEAR_WIELD, ITEM_WEAR_OFFHAND, ITEM_WEAR_TAKE, ITEM_WEAR_FLOATING,
		ITEM_WEAR_FACE
  };
  ...
  const char *already_wearing[] = {
    ...
    "You're already wielding a main hand weapon.\r\n",
    "You're already wielding an off-hand weapon.\r\n",
    "You're already holding something.\r\n",
    "You're already using a floaty.\r\n",
  ...
  if(where == WEAR_WIELD || where == WEAR_SHIELD /*|| where == WEAR_LIGHT */ || where == WEAR_HOLD || where == WEAR_OFFHAND)
	{
		...
		if (where == WEAR_SHIELD && !GET_EQ(ch, WEAR_WIELD) && GET_EQ(ch, WEAR_OFFHAND))
		{
			swap = GET_EQ(ch, WEAR_OFFHAND);
	    obj_to_char(unequip_char(ch, WEAR_OFFHAND), ch);
			wear_message(ch, swap, WEAR_WIELD);
			obj_from_char(swap);
			equip_char(ch, swap, WEAR_WIELD);
		}

    /* Will switch weapons to mainhand in order to hold an item */
    if (where == WEAR_HOLD && !GET_EQ(ch, WEAR_WIELD) && GET_EQ(ch, WEAR_OFFHAND))
    {
      swap = GET_EQ(ch, WEAR_OFFHAND);
      obj_to_char(unequip_char(ch, WEAR_OFFHAND), ch);
      wear_message(ch, swap, WEAR_WIELD);
  ...
  if (dotmode == FIND_ALL) {
    for (obj = ch->carrying; obj; obj = next_obj) {
      ...
      if (CAN_SEE_OBJ(ch, obj) && (where = find_eq_pos(ch, obj, 0)) >= 0) {
        if (GET_REAL_LEVEL(ch) < GET_OBJ_LEVEL(obj))
    ...
    if (!(obj = get_obj_in_list_vis(ch, arg1, NULL, ch->carrying)))
      send_to_char(ch, "You don't seem to have any %ss.\r\n", arg1);
    else if (GET_REAL_LEVEL(ch) < GET_OBJ_LEVEL(obj))
      send_to_char(ch, "You are not experienced enough to use that.\r\n");
    else
      while (obj) {
...
ACMD(do_claim)
{
   ...
   one_argument(argument, arg);
        ...
        if (!*arg)
          send_to_char(ch, "Claim what?\r\n");
        else if (!(obj = get_obj_in_list_vis(ch, arg, NULL, ch->carrying)))
          send_to_char(ch, "You don't seem to have %s %s.\r\n", AN(arg), arg);
        else
        {
	  if (!(obj->owner) || (obj->owner = "Undefined"))
	  {
	    send_to_char(ch,"You are now the owner of %s.\n\r",obj->short_description);
	    obj->owner = GET_NAME(ch);
	  }
	  else
	  {
	    send_to_char(ch,"This item is already owned by %s!\n\r",obj->owner);
	    return;
	  }
        }
}
  ...
  if (!*arg)
    ...
    if (!CAN_WEAR(obj, ITEM_WEAR_WIELD))
      ...
      perform_wear(ch, obj, WEAR_WIELD);
			...
			perform_wear(ch, obj, WEAR_OFFHAND);
  }
}
...
ACMD(do_sac)
{
  ...
  struct obj_data  *obj, *obj_next;  // Salty
  int gain;
  one_argument(argument, arg);
