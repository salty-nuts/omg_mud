...
static void perform_put(struct char_data *ch, struct obj_data *obj, struct obj_data *cont)
{
  ...
  else if (OBJ_FLAGGED(obj, ITEM_DECAYING))
    act("You can't put decaying objects in containers.", FALSE, ch, obj, NULL, TO_CHAR);
  else
  {
    obj_from_char(obj);
    obj_to_obj(obj, cont);
...
static void get_check_money(struct char_data *ch, struct obj_data *obj)
{
  ...
  {
    happyval = value + (long)((float)value * ((float)HAPPY_GOLD / (float)(100)));
    increase_gold(ch, happyval);
    ...
    if (GROUP(ch) && (happyval > 0) && PRF_FLAGGED(ch, PRF_AUTOSPLIT))
    {
      sprintf(buf, "%ld", happyval);
      do_split(ch, buf, 0, 0);
    }
  }
  else
  ...
  else
  {
    ...
    else
    {
      ...
      if (!str_cmp("coins", arg) || !str_cmp("coin", arg))
      {
        one_argument(argument, arg);
        if ((vict = give_find_vict(ch, arg)) != NULL)
          perform_give_gold(ch, vict, GET_GOLD(ch));
        return;
      }
      if (!ch->carrying)
