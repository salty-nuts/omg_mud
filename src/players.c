 ...
 */
    ...
    GET_TOTAL_EXP(ch) = PFDEF_TOTAL_EXP;
    GET_RANK(ch) = PFDEF_RANK;

		/* Salty Regen Eq */
		GET_HITP_REGEN(ch) = PFDEF_HITP_REGEN;
		GET_MANA_REGEN(ch) = PFDEF_MANA_REGEN;
		GET_MOVE_REGEN(ch) = PFDEF_MOVE_REGEN;
		GET_SPELLS_HEALING(ch) = PFDEF_SPELLS_HEALING;
		GET_SPELLS_DAMAGE(ch) = PFDEF_SPELLS_DAMAGE;
		GET_SPELLS_AFFECTS(ch) = PFDEF_SPELLS_DAMAGE;
	...
	else if (!strcmp(tag, "Bank"))	GET_BANK_GOLD(ch)	= atol(line);
	...
	break;
      ...
      case 'G':
	     if (!strcmp(tag, "Gold"))	GET_GOLD(ch)		= atol(line);
	break;
 ...
 */
  ...
  break;
      ...
      case 'R':
			 if (!strcmp(tag, "R_Hp"))  GET_HITP_REGEN(ch) = atoi(line);
	...
	else if (!strcmp(tag, "TEXP"))  GET_TOTAL_EXP(ch)	= atol(line);
	else if (!strcmp(tag, "Trns"))  GET_TRAINS(ch) 		= atoi(line);
  else if (!strcmp(tag, "Trnd"))  GET_TRAIN_META(ch)   = atoi(line);
 ...
 */
  ...
  if (GET_LUCK(ch)	   != PFDEF_CHA)fprintf(fl, "Luck: %d\n", GET_LUCK(ch));
  ...
  if (GET_TOTAL_EXP(ch)    != PFDEF_TOTAL_EXP)	fprintf(fl, "TEXP: %ld\n", GET_TOTAL_EXP(ch)); 
  if (GET_RANK(ch)	   != PFDEF_RANK)	fprintf(fl, "Rank: %d\n", GET_RANK(ch));
	if (GET_HITP_REGEN(ch) != PFDEF_HITP_REGEN) fprintf(fl, "R_Hp: %d\n", GET_HITP_REGEN(ch));
	if (GET_MANA_REGEN(ch) != PFDEF_MANA_REGEN) fprintf(fl, "R_Ma: %d\n", GET_MANA_REGEN(ch));
	if (GET_MOVE_REGEN(ch) != PFDEF_MOVE_REGEN) fprintf(fl, "R_Mv: %d\n", GET_MOVE_REGEN(ch));
	if (GET_SPELLS_HEALING(ch) != PFDEF_SPELLS_HEALING)	fprintf(fl, "SHea: %d\n", GET_SPELLS_HEALING(ch));
	if (GET_SPELLS_DAMAGE(ch) != PFDEF_SPELLS_DAMAGE)		fprintf(fl, "SDam: %d\n", GET_SPELLS_DAMAGE(ch));
	if (GET_SPELLS_AFFECTS(ch) != PFDEF_SPELLS_AFFECTS)	fprintf(fl, "SAff: %d\n", GET_SPELLS_AFFECTS(ch));
