...
ACMD(do_goto)
{
  ...
  if (ZONE_FLAGGED(GET_ROOM_ZONE(location), ZONE_NOIMMORT) && (GET_REAL_LEVEL(ch) >= LVL_IMMORT) && (GET_REAL_LEVEL(ch) < LVL_GRGOD)) {
...
ACMD(do_trans)
{
  ...
  if (!*buf)
    ...
    if (!(victim = get_char_vis(ch, buf, NULL, FIND_CHAR_WORLD)))
      ...
      if ((GET_REAL_LEVEL(ch) < GET_REAL_LEVEL(victim)) && !IS_NPC(victim)) {
...
static void do_stat_character(struct char_data *ch, struct char_data *k)
{
  ...
  send_to_char(ch, "%s%s, Lev: [%s%2d%s], XP: [%s%7ld%s], Align: [%4d]\r\n",
	IS_NPC(k) ? "Mobile" : "Class: ", IS_NPC(k) ? "" : buf, CCYEL(ch, C_NRM), GET_REAL_LEVEL(k), CCNRM(ch, C_NRM),
  ...
  if (!IS_NPC(k)) {
    ...
    if (GET_REAL_LEVEL(k) >= LVL_BUILDER) {
  ...
  send_to_char(ch, "Gold: [%12ld], Bank: [%12ld] (Total: %12ld), ",
  ...
  send_to_char(ch, "AC: [%d%+d/10], Hitroll: [%2d], Damroll: [%2d], Saving throws: [%d/%d/%d/%d/%d]\r\n",
	  GET_AC(k), dex_app[GET_DEX(k)].defensive, GET_HITROLL(k),
	  GET_DAMROLL(k), GET_SAVE(k, 0), GET_SAVE(k, 1), GET_SAVE(k, 2),
	  GET_SAVE(k, 3), GET_SAVE(k, 4));

  int tmp_class = GET_CLASS(k);
  int tmp_level = GET_REAL_LEVEL(k);
  int t0 = saving_throws(tmp_class, GET_SAVE(k, 0), tmp_level) + GET_SAVE(k,0);
  int t1 = saving_throws(tmp_class, GET_SAVE(k, 1), tmp_level) + GET_SAVE(k,1);
  int t2 = saving_throws(tmp_class, GET_SAVE(k, 2), tmp_level) + GET_SAVE(k,2);
  int t3 = saving_throws(tmp_class, GET_SAVE(k, 3), tmp_level) + GET_SAVE(k,3);
  int t4 = saving_throws(tmp_class, GET_SAVE(k, 4), tmp_level) + GET_SAVE(k,4);
  ...
  if (IS_NPC(k))
  {
    ...
    send_to_char(ch, ", Number of Attacks: %d", k->mob_specials.attack_num);
  }
  if (k->desc) {
...
//  send_to_char(ch, "Max weight: %d, Max items: %d\r\n" CAN_CARRY_W(k), CAN_CARRY_N(k));
  ...
  if (!IS_NPC(k) && (GET_REAL_LEVEL(k) >= LVL_IMMORT)) {
    if (POOFIN(k))
...
void snoop_check(struct char_data *ch)
{
  ...
  if (!ch || !ch->desc)
    return;
  if (ch->desc->snooping &&
     (GET_REAL_LEVEL(ch->desc->snooping->character) >= GET_REAL_LEVEL(ch))) {
    ch->desc->snooping->snoop_by = NULL;
    ch->desc->snooping = NULL;
  }

  if (ch->desc->snoop_by &&
     (GET_REAL_LEVEL(ch) >= GET_REAL_LEVEL(ch->desc->snoop_by->character))) {
...
static void stop_snooping(struct char_data *ch)
{
  if (!ch->desc->snooping)
    ...
    send_to_char(ch, "You stop snooping.\r\n");
      ...
      mudlog(BRF, GET_REAL_LEVEL(ch), TRUE, "(GC) %s stops snooping", GET_NAME(ch));
...
ACMD(do_snoop)
{
  ...
  if (!*arg)
    ...
    if (GET_REAL_LEVEL(tch) >= GET_REAL_LEVEL(ch)) {
      send_to_char(ch, "You can't.\r\n");
      return;
    }
    send_to_char(ch, "%s", CONFIG_OK);

      mudlog(BRF, GET_REAL_LEVEL(ch), TRUE, "(GC) %s snoops %s", GET_NAME(ch), GET_NAME(victim));
...
ACMD(do_switch)
{
  ...
  else if ((GET_REAL_LEVEL(ch) < LVL_IMPL) && !IS_NPC(victim))
    send_to_char(ch, "You are not holy enough to use their body.\r\n");
  else if (GET_REAL_LEVEL(ch) < LVL_GRGOD && ROOM_FLAGGED(IN_ROOM(victim), ROOM_GODROOM))
    send_to_char(ch, "You are not godly enough to use that room!\r\n");
  else if (GET_REAL_LEVEL(ch) < LVL_GRGOD && ROOM_FLAGGED(IN_ROOM(victim), ROOM_HOUSE)
...
static void do_cheat(struct char_data *ch)
{
  ...
  {
    case    1: // IMP
      GET_LEVEL(ch, GET_CLASS(ch)) = LVL_IMPL;
      break;
    default:
...
ACMD(do_return)
{
  if (!IS_NPC(ch) && !ch->desc->original) {
    int level, newlevel;
    level = GET_REAL_LEVEL(ch);
    do_cheat(ch);
    newlevel = GET_REAL_LEVEL(ch);
...
ACMD(do_load)
{
  ...
  if (is_abbrev(buf, "mob")) {
	...
	if (GET_REAL_LEVEL(ch) < LVL_GRGOD && !can_edit_zone(ch, world[IN_ROOM(ch)].zone)) {
...
ACMD(do_purge)
{
  ...
  if (*buf) {
    ...
    if ((vict = get_char_vis(ch, buf, &number, FIND_CHAR_ROOM)) != NULL) {      
      if (!IS_NPC(vict) && (GET_REAL_LEVEL(ch) <= GET_REAL_LEVEL(vict))) {
...
ACMD(do_advance)
{
  ...
  int newlevel, oldlevel, altered = FALSE;
  int diff;
  ...
  if (GET_REAL_LEVEL(ch) <= GET_REAL_LEVEL(victim)) {
  ...
  send_to_char(ch,"The difference is %d\n\r",diff);

  if (diff > 0)
  {
    do_start(victim);
    GET_LEVEL(victim, GET_CLASS(victim)) = newlevel;
  ...
  {
    act("$n makes some strange gestures. You feel slightly different.",FALSE, ch, 0, victim, TO_VICT);
    for (int i = 0;i < newlevel - oldlevel;i++)
    {
      GET_LEVEL(victim, GET_CLASS(victim)) += 1;
...
ACMD(do_restore)
{
  ...
  if (!*buf)
   ...
   else if (is_abbrev(buf, "all"))
   {
    ...
    {
      if (!IS_PLAYING(j) || !(vict = j->character) || GET_REAL_LEVEL(vict) >= LVL_IMMORT)
     continue;
    ...
    send_to_char(ch, "%s", CONFIG_NOPERSON);
  else if (!IS_NPC(vict) && ch != vict && GET_REAL_LEVEL(vict) >= GET_REAL_LEVEL(ch))
    ...
    if (!IS_NPC(vict) && GET_REAL_LEVEL(ch) >= LVL_GRGOD) {
      if (GET_REAL_LEVEL(vict) >= LVL_IMMORT)
        for (i = 1; i <= MAX_SKILLS; i++)
          SET_SKILL(vict, i, 100);

      if (GET_REAL_LEVEL(vict) >= LVL_GRGOD) {
...
ACMD(do_dc)
{
  ...
  if (d->character && GET_REAL_LEVEL(d->character) >= GET_REAL_LEVEL(ch)) {
...
ACMD(do_wizlock)
{
  ...
  if (*arg) {
    value = atoi(arg);
    if (value < 0 || value > GET_REAL_LEVEL(ch)) {
...
ACMD(do_last)
{
  ...
  *name = '\0';
  ...
  if (*argument) { /* parse it */
    ...
    while (*arg) {
      if ((*arg == '*' || !strcmp(arg, "all")) && (GET_REAL_LEVEL(ch) == LVL_IMPL)) {
  ...
  if (*name && !num) {
    ...
    GET_IDNUM(vict), (int) GET_REAL_LEVEL(vict),
...
ACMD(do_force)
{
  ...
  if (!*arg || !*to_force)
  ...
  else if ((GET_REAL_LEVEL(ch) < LVL_GRGOD) || (str_cmp("all", arg) && str_cmp("room", arg))) {
    if (!(vict = get_char_vis(ch, arg, NULL, FIND_CHAR_WORLD)))
      send_to_char(ch, "%s", CONFIG_NOPERSON);
    else if (!IS_NPC(vict) && GET_REAL_LEVEL(ch) < LVL_GOD)
      send_to_char(ch, "You cannot force players.\r\n");
    else if (!IS_NPC(vict) && GET_REAL_LEVEL(ch) <= GET_REAL_LEVEL(vict))
    ...
    for (vict = world[IN_ROOM(ch)].people; vict; vict = next_force) {
      ...
      if (!IS_NPC(vict) && GET_REAL_LEVEL(vict) >= GET_REAL_LEVEL(ch))
	continue;
      act(buf1, TRUE, ch, NULL, vict, TO_VICT);
      command_interpreter(vict, to_force);
    }
    ...
    for (i = descriptor_list; i; i = next_desc) {
      ...
      if (STATE(i) != CON_PLAYING || !(vict = i->character) || (!IS_NPC(vict) && GET_REAL_LEVEL(vict) >= GET_REAL_LEVEL(ch)))
	continue;
      act(buf1, TRUE, ch, NULL, vict, TO_VICT);
      command_interpreter(vict, to_force);
    }
...
ACMD(do_wiznet)
{
  ...
  case '@':
    ...
    for (d = descriptor_list; d; d = d->next) {
      if (STATE(d) != CON_PLAYING || GET_REAL_LEVEL(d->character) < LVL_IMMORT)
        continue;
      if (!CAN_SEE(ch, d->character))
        continue;
  ...
  for (d = descriptor_list; d; d = d->next) {
    if (IS_PLAYING(d) && (GET_REAL_LEVEL(d->character) >= level) &&
...
ACMD(do_zreset)
{
  ...
  if (i <= top_of_zone_table && (can_edit_zone(ch, i) || GET_REAL_LEVEL(ch) > LVL_IMMORT)) {
    reset_zone(i);
...
ACMD(do_wizutil)
{
  ...
  if (!*arg)
  ...
  else if (GET_REAL_LEVEL(vict) >= GET_REAL_LEVEL(ch) && vict != ch)
    ...
    case SCMD_REROLL:
      ...
      send_to_char(ch, "New stats: Str %d/%d, Int %d, Wis %d, Dex %d, Con %d, Luck %d\r\n",
	      GET_STR(vict), GET_ADD(vict), GET_INT(vict), GET_WIS(vict),
	      GET_DEX(vict), GET_CON(vict), GET_LUCK(vict));
      break;
    case SCMD_PARDON:
	...
	return;
      ...
      GET_FREEZE_LEV(vict) = GET_REAL_LEVEL(ch);
	...
	return;
      ...
      if (GET_FREEZE_LEV(vict) > GET_REAL_LEVEL(ch)) {
...
ACMD(do_show)
{
  ...
  if (!*argument) {
    ...
    for (j = 0, i = 1; fields[i].level; i++)
      if (fields[i].level <= GET_REAL_LEVEL(ch))
  ...
  if (GET_REAL_LEVEL(ch) < fields[l].level) {
  ...
  case 2: {
    ...
    send_to_char(ch, "Player: %-12s (%s) [%2d %s]\r\n", GET_NAME(vict),
      genders[(int) GET_SEX(vict)], GET_REAL_LEVEL(vict), class_abbrevs[(int)
      GET_CLASS(vict)]);
  ...
  case 10:
	...
	continue;
      if (STATE(d) != CON_PLAYING || GET_REAL_LEVEL(ch) < GET_REAL_LEVEL(d->character))
	continue;
  ...
  case 11:
    ...
    for (j = 1; j < LVL_IMMORT; j++) {
      nlen = snprintf(buf + len, sizeof(buf) - len,  "%-3d - %-2d %-2d %-2d %-2d %-2d\r\n", j, 
				thaco(CLASS_WIZARD, j),
				thaco(CLASS_PRIEST, j),
				thaco(CLASS_ROGUE, j),
				thaco(CLASS_FIGHTER, j),
				thaco(CLASS_BARD, j));
      if (len + nlen >= sizeof(buf))
        break;
      len += nlen;
  ...
  case 12:
    ...
    for (i = 1; i < LVL_IMMORT; i++) { 
      nlen = snprintf(buf + len, sizeof(buf) - len,  "%-3d - %-6d %-6d %-6d %-6d %-6d\r\n", i,  
				level_exp(CLASS_WIZARD, i) - level_exp(CLASS_WIZARD, i - 1),
				level_exp(CLASS_PRIEST, i) - level_exp(CLASS_PRIEST, i - 1),
...
static struct set_struct {
   ...
   { "exp", 		LVL_GOD, 	BOTH, 	LNUMBER },
   { "frozen",		LVL_GRGOD, 	PC,	BINARY },  /* 15 */
   { "gold",		LVL_BUILDER, 	BOTH, 	LNUMBER },
   ...
   { "totalexp",	LVL_BUILDER,	PC,	LNUMBER},
   { "multiclass",     LVL_IMPL,    PC,    BINARY},
   { "resist_unarmed", LVL_IMPL,    PC,    NUMBER},
   { "resist_exotic", LVL_IMPL,    PC,    NUMBER},
   { "resist_blunt", LVL_IMPL,    PC,    NUMBER},
   { "resist_pierce", LVL_IMPL,    PC,    NUMBER},
   { "resist_slash", LVL_IMPL,    PC,    NUMBER},
	 { "hitp_regen", LVL_IMPL, PC, NUMBER},
...
static int perform_set(struct char_data *ch, struct char_data *vict, int mode, char *val_arg)
{
  ...
  if (GET_REAL_LEVEL(ch) != LVL_IMPL) {
    if (!IS_NPC(vict) && GET_REAL_LEVEL(ch) <= GET_REAL_LEVEL(vict) && vict != ch) {
      send_to_char(ch, "Maybe that's not such a great idea...\r\n");
      return (0);
    }
  }
  if (GET_REAL_LEVEL(ch) < set_fields[mode].level) {
    send_to_char(ch, "You are not godly enough for that!\r\n");
    return (0);
  }
  ...
  else if (set_fields[mode].type == LNUMBER)
  {
    lvalue = atol(val_arg);
  }
  else if (set_fields[mode].type == NUMBER)
 {
  ...
  switch (mode)
  {
   ...
   case 21: /* int */
      ...
      break;
    case 22: /* invis */
      if (GET_REAL_LEVEL(ch) < LVL_IMPL && ch != vict) {
        send_to_char(ch, "You aren't godly enough for that!\r\n");
        return (0);
      }
      GET_INVIS_LEV(vict) = RANGE(0, GET_REAL_LEVEL(vict));
      break;
    case 23: /* invistart */
    ...
    case 24: /* killer */
      ...
      break;
    case 25: /* level */
      if ((!IS_NPC(vict) && value > GET_REAL_LEVEL(ch)) || value > LVL_IMPL) {
        send_to_char(ch, "You can't do that.\r\n");
        return (0);
      }
      RANGE(1, LVL_IMPL);
//      vict->player.level = value;
	send_to_char(ch, "Multiclass set not implemented yet.\n\r");
      break;
    case 26: /* loadroom */
      if (!str_cmp(val_arg, "off")) {
    ...
    case 31: /* move */
      ...
      break;
    case 32: /* name */
      if (ch != vict && GET_REAL_LEVEL(ch) < LVL_IMPL) {
    ...
    case 33: /* nodelete */
      ...
      break;
    case 34: /* nohassle */
      if (GET_REAL_LEVEL(ch) < LVL_GOD && ch != vict) {
        send_to_char(ch, "You aren't godly enough for that!\r\n");
        return (0);
      }
    ...
    case 38: /* password */
      ...
      *(GET_PASSWD(vict) + MAX_PWD_LENGTH) = '\0';
      ...
      break;
    case 39: /* poofin */
      if ((vict == ch) || (GET_REAL_LEVEL(ch) == LVL_IMPL)) {
        skip_spaces(&val_arg);
        parse_at(val_arg);
    ...
    case 58: /* totalexp */
      GET_TOTAL_EXP(vict) = lvalue;
      break;
    case 59: /* Salty's Multiclass */
      SET_OR_REMOVE(PLR_FLAGS(vict), PLR_MULTICLASS);
      affect_total(vict);
      break;
		case 60:
      RANGE(-1000, 1000);
      ...
      affect_total(vict);
		...
		case 65:
			RANGE(0,1000);
			GET_HITP_REGEN(vict) = value;
			affect_total(vict);
			break;
		case 66:
			RANGE(0,1000);
			GET_MANA_REGEN(vict) = value;
			affect_total(vict);
			break;
		case 67:
			RANGE(0,1000);
			GET_MOVE_REGEN(vict) = value;
			affect_total(vict);
			break;
                case 68:
                        RANGE(0,5000);
                        GET_TRAINS(vict) = value;
                        affect_total(vict);
                        break;
		case 69:
			RANGE(0,1000);
  ...
  {
    send_to_char(ch, "%s's %s set to %ld.\r\n", GET_NAME(vict), set_fields[mode].cmd, lvalue);
  }
  else
    send_to_char(ch, "%s", CONFIG_OK);
...
{
/*
  Replaced with set_fields[i].level
  Salty, 13 JAN 2019
...
*/
  ...
  for (i = 0; *(set_fields[i].cmd) != '\n'; i++)
  {
    ...
    if (set_fields[i].level <= GET_REAL_LEVEL(ch))
    {
      add_len = snprintf(buf+len, sizeof(buf)-len, "%-20s%3d  %-4s  %-6s\r\n", set_fields[i].cmd,
...
ACMD(do_set)
{
  ...
  if (!*name || !*field) {
    ...
    send_to_char(ch, "       %shelp set%s will display valid fields\r\n", CCYEL(ch, C_NRM), CCNRM(ch, C_NRM));
    return;
  }
  ...
  if (!is_file) {
    ...
    if ((player_i = load_char(name, cbuf)) > -1) {
      if (GET_REAL_LEVEL(cbuf) > GET_REAL_LEVEL(ch)) {
	free_char(cbuf);
	send_to_char(ch, "Sorry, you can't do that.\r\n");
	return;
      }
...
/*mob limits*/
#define MAX_DAMROLL_ALLOWED      MAX(GET_REAL_LEVEL(mob)/5, 1)
#define MAX_HITROLL_ALLOWED      MAX(GET_REAL_LEVEL(mob)/3, 1)
#define MAX_MOB_GOLD_ALLOWED     GET_REAL_LEVEL(mob)*3000
#define MAX_EXP_ALLOWED          GET_REAL_LEVEL(mob)*GET_REAL_LEVEL(mob) * 120
...
/* Armor class limits*/
#define TOTAL_WEAR_CHECKS  (NUM_ITEM_WEARS-3)  /*minus Wield, Off-hand and Take*/
...
static struct zcheck_affs {
  ...
  {APPLY_HITP_REGEN,	-5,   20, "bonus hitpoint regen"},
  {APPLY_MANA_REGEN,	-5,   20, "bonus mana regen"},
  {APPLY_MOVE_REGEN,	-5,   20, "bonus movement regen"},
  {APPLY_SPELLS_HEALING,	-5,   20, "healing spell bonus"},
  {APPLY_SPELLS_DAMAGE,		-5,   20, "damage spell bonus"},
  {APPLY_SPELLS_AFFECTS,	-5,   20, "affects spell bonus"},
	{APPLY_MIRROR_IMAGE,	0, 99, "mirror images"},
	{APPLY_RESIST_UNARMED,0,	20,	"resist unarmed"},
	{APPLY_RESIST_EXOTIC, 0,	20,	"resist exotic"},
	{APPLY_RESIST_BLUNT,	0,	20, "resist blunt"},
	{APPLY_RESIST_PIERCE,	0,	20, "resist pierce"},
	{APPLY_RESIST_SLASH,	0,	20, "resist slash"}
};
 ...
 /* Check mobs */
  ...
  for (i=0; i<top_of_mobt;i++) {
      if (real_zone_by_thing(mob_index[i].vnum) == zrnum) {  /*is mob in this zone?*/
        ...
        if (GET_REAL_LEVEL(mob)>MAX_LEVEL_ALLOWED && (found=1))
          len += snprintf(buf + len, sizeof(buf) - len,
                          "- Is level %d (limit: 1-%d)\r\n",
                          GET_REAL_LEVEL(mob), MAX_LEVEL_ALLOWED);
...
ACMD(do_zpurge)
{
  ...
  if (GET_REAL_LEVEL(ch) < LVL_GOD && !can_edit_zone(ch, zone)) {
...
ACMD(do_zlock)
{
  ...
  if (GET_REAL_LEVEL(ch) < LVL_GRGOD && !is_name(GET_NAME(ch), zone_table[zn].builders) && GET_OLC_ZONE(ch) != znvnum) {
...
ACMD(do_recent)
{
  ...
  if (GET_REAL_LEVEL(ch) >= LVL_GRGOD) {  /* If High-Level Imm, then show Host IP */
