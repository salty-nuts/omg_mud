...
static void do_stat_object(struct char_data *ch, struct obj_data *j)
{
  ...
  case ITEM_KEY: /* Nothing */
    send_to_char(ch, "Chance to break when used: %d\n\r", GET_OBJ_VAL(j, 0));
    break;
  case ITEM_FOOD:
...
static void do_stat_character(struct char_data *ch, struct char_data *k)
{
  ...
  int tmp_level = GET_LEVEL(k);
  int t0 = saving_throws(k, GET_SAVE(k, 0), tmp_level) + GET_SAVE(k, 0);
  int t1 = saving_throws(k, GET_SAVE(k, 1), tmp_level) + GET_SAVE(k, 1);
  int t2 = saving_throws(k, GET_SAVE(k, 2), tmp_level) + GET_SAVE(k, 2);
  int t3 = saving_throws(k, GET_SAVE(k, 3), tmp_level) + GET_SAVE(k, 3);
  int t4 = saving_throws(k, GET_SAVE(k, 4), tmp_level) + GET_SAVE(k, 4);
...
static struct set_struct
{
  ...
  const char type;
    ...
    {"multiclass (removed)", LVL_IMPL, PC, BINARY},
    ...
    {"combat_power", LVL_IMPL, PC, NUMBER},
...
static int perform_set(struct char_data *ch, struct char_data *vict, int mode, char *val_arg)
{
  ...
  case 58: /* totalexp */
    ...
    break;
  case 59: /* Salty's Multiclass REMOVED*/
    break;
  case 60:
    RANGE(-1000, 1000);
