...
ASPELL(spell_summon)
{
  ...
  if (MOB_FLAGGED(victim, MOB_NOSUMMON) ||
      (IS_NPC(victim) && mag_savingthrow(victim, ch, SAVING_SPELL, SPELL_SUMMON, 0)))
  {
    send_to_char(ch, "%s", SUMMON_FAIL);
    return;
  }
...
ASPELL(spell_charm)
{
  ...
  else if (mag_savingthrow(victim, ch, SAVING_PARA, SPELL_CHARM, 0))
    send_to_char(ch, "Your victim resists!\r\n");
  else
  {
    if (victim->master)
...
ASPELL(spell_identify)
{
  ...
  if (obj)
  {
    ...
    send_to_char(ch, "Weight: %d, Value: %d, Timer, %d Rent: %d, Min. level: %d\r\n",
                 GET_OBJ_WEIGHT(obj), GET_OBJ_COST(obj), GET_OBJ_TIMER(obj), GET_OBJ_RENT(obj), GET_OBJ_LEVEL(obj));
...
ASPELL(spell_mana_transfer)
{
  ...
  {
    send_to_char(ch, "%s has full mana!\n\r", GET_NAME(victim));
    return;
  }
  else
  {
    send_to_char(victim, "%s transfers some mana to you!\n\r", GET_NAME(ch));
    GET_MANA(victim) += managain;
    return;
  }
}
...
EVENTFUNC(event_chain_lightning)
{
  ...
  call_magic(ch, NULL, NULL, SPELL_CHAIN_LIGHTNING, GET_LEVEL(ch) + GET_SPELLS_DAMAGE(ch), CAST_SPELL);
  return 0;
}
