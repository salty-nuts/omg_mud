...
/*
 * Salty Prompt Additions (Rewrite)
 * 01 FEB 2020
 */
static char *make_prompt(struct descriptor_data *d)
{
  ...
  if (d->showstr_count)
    ...
    *prompt = '\0';
    ...
    /* AUTO PROMPT - show only when below 25% */
    ...
    else /* NOT AUTO PROMPT */
    {
      ...
      {
        count = snprintf(prompt + len, sizeof(prompt) - len, "%d", GET_HIT(d->character));
        if (count >= 0)
          len += count;

	if (PRF_FLAGGED(d->character, PRF_DISP_MAXHP) && len < sizeof(prompt))
	  {
	    count = snprintf(prompt + len, sizeof(prompt) - len, "(%d)", GET_MAX_HIT(d->character));
	    if (count >= 0)
	      len += count;
	  }
	count = snprintf(prompt + len, sizeof(prompt) - len, "hp ");
	len += count;
      }

      if (PRF_FLAGGED(d->character, PRF_DISPMANA) && len < sizeof(prompt))
      {
        count = snprintf(prompt + len, sizeof(prompt) - len, "%d", GET_MANA(d->character));
        if (count >= 0)
          len += count;

	if (PRF_FLAGGED(d->character, PRF_DISP_MAXMANA) && len < sizeof(prompt))
	  {
	    count = snprintf(prompt + len, sizeof(prompt) - len, "(%d)", GET_MAX_MANA(d->character));
	    if (count >= 0)
	      len += count;
	  }
        count = snprintf(prompt + len, sizeof(prompt) - len, "mana ");
        len += count;
      ...
      {
        count = snprintf(prompt + len, sizeof(prompt) - len, "%d", GET_MOVE(d->character));
        if (count >= 0)
          len += count;

	if (PRF_FLAGGED(d->character, PRF_DISP_MAXMOVE) && len < sizeof(prompt))
	{
	  count = snprintf(prompt + len, sizeof(prompt) - len, "(%d)", GET_MAX_MOVE(d->character));
	  if (count >= 0)
	    len += count;
	}
	count = snprintf(prompt + len, sizeof(prompt) - len, "mv ");
        len += count;
      }
    }
