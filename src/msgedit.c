  ...
  switch (OLC_MODE(d))
  {
    case MSGEDIT_MAIN_MENU:
          ...
          if (OLC_MSG(d)->next == NULL)
	  {
        ...
        return;
	case '>':
	  write_to_output(d, "Example: number of attacks\n\r");
	  write_to_output(d, "Enter new number of attacks: ");
	  OLC_MODE(d) = MSGEDIT_NUM_ATTACKS;
	return;
      }
    break;
    case MSGEDIT_CONFIRM_SAVE:
      ...
      msgedit_main_menu(d);
    return;
    case MSGEDIT_NUM_ATTACKS:
      OLC_MSG_LIST(d)->number_of_attacks = MAX(atoi(arg),1);
    break;
    case MSGEDIT_TYPE:
