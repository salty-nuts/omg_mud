...
static void aff_apply_modify(struct char_data *ch, byte loc, sh_int mod, char *msg)
{
  switch (loc) {
  case APPLY_NONE:
    break;
...
struct char_data *get_char_room_vis(struct char_data *ch, char *name, int *number)
{
  ...
  /* JE */
  if (!str_cmp(name, "self") || !str_cmp(name, "me") || !str_cmp(name,"group"))
    return (ch);
...
const char *money_desc(long amount)
{
  int cnt;
  struct {
    int limit;
    ...
    {   10000000, "a gigantic mountain of gold coins"	},
    {  100000000, "a colossal mountain of gold coins"	},
    { 1000000000, "a titanic mountain of gold coins"	},
    {          0, NULL					},
  };
