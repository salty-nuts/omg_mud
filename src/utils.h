 ...
 * Salty, 14 JAN 2019
 */
char* add_commas(long long int X);
...
long decrease_gold(struct char_data *ch, long amt);
long increase_bank(struct char_data *ch, long amt);
long decrease_bank(struct char_data *ch, long amt);
...
/*
  I want some defines for real ability scores
  Salty
  06 JAN 2019
*/
...
#define GET_REAL_LUCK(ch)     ((ch)->real_abils.luck)
...
/** Number of stat train sesssions remaining for ch. */
#define GET_TRAINS(ch)	        CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.stat_trains))
#define GET_TRAIN_META(ch)         CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.stat_trained))

/*  Defines for Hitp, Mana, Move regen */
#define GET_HITP_REGEN(ch)	        CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.hitp_regen))
#define GET_MANA_REGEN(ch)	        CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.mana_regen))
#define GET_MOVE_REGEN(ch)	        CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.move_regen))

#define GET_SPELLS_HEALING(ch)			CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.spells_healing))
#define GET_SPELLS_DAMAGE(ch)				CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.spells_damage))
#define GET_SPELLS_AFFECTS(ch)			CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.spells_affects))
...
/** Return the equivalent strength of ch if ch has level 25 strength. */
#define STRENGTH_APPLY_INDEX(ch) \
        ( ((GET_ADD(ch) ==0) || (GET_STR(ch) != 25)) ? GET_STR(ch) :\
          (GET_ADD(ch) <= 50) ? 26 :( \
          (GET_ADD(ch) <= 75) ? 27 :( \
          (GET_ADD(ch) <= 90) ? 28 :( \
          (GET_ADD(ch) <= 99) ? 29 :  30 ) ) )                   \
...
/* Salty's Multiclass */
#define MULTI_CLASS_ABBR(ch) (IS_NPC(ch) ? "---" : class_abbrevs[(int)GET_MULTI_CLASS(ch)])
...
/* Get the Mortal Recall room */
#define CONFIG_RECALL_ROOM			config_info.room_nums.recall_room
