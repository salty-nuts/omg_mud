...
ACMD(do_quit)
{
  ...
  if (subcmd != SCMD_QUIT && GET_REAL_LEVEL(ch) < LVL_IMMORT)
    ...
    mudlog(NRM, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE, "%s has quit the game.", GET_NAME(ch));
		...
		game_info("%s has left the game.",GET_NAME(ch));
...
ACMD(do_steal)
{
  ...
  if (str_cmp(obj_name, "coins") && str_cmp(obj_name, "gold")) {
	...
	act("$n tries to steal something from $N.", TRUE, ch, 0, vict, TO_NOTVICT);
        check_improve(ch,SKILL_STEAL,FALSE);
      } else {			/* Steal the item */
	if (IS_CARRYING_N(ch) + 1 < CAN_CARRY_N(ch)) {
	  ...
	  if (IS_CARRYING_W(ch) + GET_OBJ_WEIGHT(obj) < CAN_CARRY_W(ch)) {
	    ...
	    check_improve(ch,SKILL_STEAL,TRUE);
	  }
	} else
...
static void print_group(struct char_data *ch)
{
  ...
  while ((k = (struct char_data *) simple_list(ch->group->members)) != NULL)
    send_to_char(ch, "%-*s: %s[%5d/%-5d]H [%5d/%-5d]M [%5d/%-5d]V%s\r\n",
	    MAX_NAME_LENGTH, GET_NAME(k), 
...
ACMD(do_split)
{
  ...
  if (is_long_int(buf))
  {
    lamount = atol(buf);
    if (lamount <= 0) {
      send_to_char(ch, "Sorry, you can't do that.  Value is less than or equal to zero.\r\n");
      return;
    }
    if (lamount > GET_GOLD(ch))
    {
    ...
    len = snprintf(buf, sizeof(buf), "%s splits %s coins; you receive %s.\r\n",
		GET_NAME(ch), add_commas(lamount), add_commas(share));
    if (rest && len < sizeof(buf)) {
      snprintf(buf + len, sizeof(buf) - len,
		"%ld coin%s %s not splitable, so %s keeps the money.\r\n", rest,
    ...
    send_to_char(ch, "You split %s coins among %ld members -- %s coins each.\r\n",
	    add_commas(lamount), num, add_commas(share));

    if (rest) {
      send_to_char(ch, "%ld coin%s %s not splitable, so you keep the money.\r\n",
