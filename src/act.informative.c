...
bool PrintTemporaryAffects(char_data *ch, char *msg)
{
  struct affected_type *aff;
  char output[MAX_STRING_LENGTH] = {0}, temp[MAX_STRING_LENGTH];
...
07 NOV 2019
*/
//        sprintf(temp, "  %-20s expires in %3d hours\r\n", spell_info[aff->spell].name, aff->duration);
        sprintf(temp, "  %15s modifies %17s by %4d for %4d hours\r\n", spell_info[aff->spell].name, affect_loc_name(aff->location), aff->modifier, aff->duration);
        strcat(output, temp);
    }
  }
  ...
  strcpy(msg, "You are not affected by any temporary spell.\r\n");
  return true;
}

bool PrintPermanentAffects(char_data *ch, char *msg)
{
  struct affected_type *aff;
  char output[MAX_STRING_LENGTH] = {0};

  for (aff = ch->affected; aff; aff=aff->next)
    affect_modify_ar(ch, aff->location, aff->modifier, aff->bitvector, false);

  if (AFF_FLAGGED(ch, AFF_HARMONY))
    strcat(output, "  Harmonic Chant\n\r");
  if (AFF_FLAGGED(ch, AFF_DISSONANCE))
    strcat(output, "  Dissonant Chant\n\r");    
  if (AFF_FLAGGED(ch, AFF_DEFENSE))
    strcat(output, "  Defensive Stance\r\n");
  if (AFF_FLAGGED(ch, AFF_OFFENSE))
    strcat(output, "  Offensive Stance\n\r");
  if (AFF_FLAGGED(ch, AFF_DETECT_ALIGN))
    strcat(output, "  Detect-alignment\r\n");
  if (AFF_FLAGGED(ch, AFF_DETECT_INVIS))
    strcat(output, "  Detect-invisible\r\n");
  if (AFF_FLAGGED(ch, AFF_DETECT_MAGIC))
    strcat(output, "  Detect-magic\r\n");
  ...
  if (AFF_FLAGGED(ch, AFF_IMP_PASS_DOOR))
    strcat(output, "  Imp Pass Door\r\n");
  if (AFF_FLAGGED(ch, AFF_PASS_DOOR))
    strcat(output, "  Pass Door\r\n");
  if (AFF_FLAGGED(ch, AFF_QUICKCAST))
    strcat(output, "  Quickcast\r\n");
  if (AFF_FLAGGED(ch, AFF_IMPROVED_INVIS))
    strcat(output, "  Improved Invis\r\n");
  if (AFF_FLAGGED(ch, AFF_INVISIBLE))
    strcat(output, "  Invisible\r\n");
  if (AFF_FLAGGED(ch, AFF_SANCTUARY))
    strcat(output, "  Sanctuary\r\n");
  ...
  if (AFF_FLAGGED(ch, AFF_FLYING))
    strcat(output, "  Fly\r\n");
  for (aff = ch->affected; aff; aff=aff->next)
    affect_modify_ar(ch, aff->location, aff->modifier, aff->bitvector, true);

  if (output[0])
  {
    sprintf(msg, "Permanent Affects:\r\n%s\r\n", output);
    return true;
  }
  return false;
}
...
ACMD(do_affect)
{
  ...
  if (PrintPermanentAffects(ch,msg))
    send_to_char(ch, "%s\n\r", msg);

  if (PrintTemporaryAffects(ch, msg))
    send_to_char(ch, "%s\n\r", msg);
}
...
static void look_at_char(struct char_data *i, struct char_data *ch)
{
  ...
  if (ch != i && (IS_ROGUE(ch) || GET_REAL_LEVEL(ch) >= LVL_IMMORT)) {
...
static void list_one_char(struct char_data *i, struct char_data *ch)
{
  ...
  if (IS_NPC(i) && i->player.long_descr && GET_POS(i) == GET_DEFAULT_POS(i) && GET_POS(i) != POS_FIGHTING)
  {
    ...
    if (AFF_FLAGGED(i, AFF_HARMONY))
      act("...$n is singing a haromnic chant!", FALSE, i, 0, ch, TO_VICT);
    if ( AFF_FLAGGED(i, AFF_DISSONANCE))
      act("...$n is singing a dissonant chant!", FALSE, i, 0, ch, TO_VICT);
    ...
    if (AFF_FLAGGED(i, AFF_RAGE))
      act("...$n is abosolutely RAGING!", FALSE, i, 0, ch, TO_VICT);
    if (AFF_FLAGGED(i, AFF_BLIND) && GET_REAL_LEVEL(i) < LVL_IMMORT)
  ...
  {
    if (GET_RANK(i) > 49)
      act("...$e wears a magnificent golden crown!", FALSE, i, 0, ch, TO_VICT);
    else if (GET_RANK(i) > 39)
      act("...$e wears a jewelled silver circlet!", FALSE, i, 0, ch, TO_VICT);
    else if (GET_RANK(i) > 29)
      act("...$e wears a coronet of strawberries!", FALSE, i, 0, ch, TO_VICT);
    else if (GET_RANK(i) > 19)
      act("...$e wears a plain silver circlet!", FALSE, i, 0, ch, TO_VICT);
    else if (GET_RANK(i) > 9)
      act("...$e wears a crown of olive branches!", FALSE, i, 0, ch, TO_VICT);
  }
  if (AFF_FLAGGED(i, AFF_OFFENSE))
    act("...$n is in an offensive fighting stance!", FALSE, i, 0, ch, TO_VICT);
  if (AFF_FLAGGED(i, AFF_DEFENSE))
...
static void do_auto_exits(struct char_data *ch)
{
  ...
  if (PRF_FLAGGED(ch, PRF_AUTOEXIT_L))
  {
     do_exits(ch,"",0,0);
     return;
  }
...
ACMD(do_exits)
{
  ...
  if (AFF_FLAGGED(ch, AFF_BLIND) && GET_REAL_LEVEL(ch) < LVL_IMMORT) {
...
void look_at_room(struct char_data *ch, int ignore_brief)
{
  ...
  if (IS_DARK(IN_ROOM(ch)) && !CAN_SEE_IN_DARK(ch)) {
    ...
    return;
  } else if (AFF_FLAGGED(ch, AFF_BLIND) && GET_REAL_LEVEL(ch) < LVL_IMMORT) {
...
static void look_in_obj(struct char_data *ch, char *arg)
{
  ...
  if (!*arg)
    ...
    if (GET_OBJ_TYPE(obj) == ITEM_CONTAINER) {
      if (OBJVAL_FLAGGED(obj, CONT_CLOSED) && (GET_REAL_LEVEL(ch) < LVL_IMMORT || !PRF_FLAGGED(ch, PRF_NOHASSLE)))
...
ACMD(do_score)
{
  ...
  send_to_char(ch, "You are known as %s %s (level: %d)\n\r",GET_NAME(ch), GET_TITLE(ch),GET_REAL_LEVEL(ch));

	if (PLR_FLAGGED(ch, PLR_MULTICLASS))
		send_to_char(ch, "You are a level %d %s, your multiclass is a level %d %s.\n\r",
			GET_LEVEL(ch, GET_CLASS(ch)), pc_class_types[GET_CLASS(ch)], GET_LEVEL(ch, GET_MULTI_CLASS(ch)), pc_class_types[GET_MULTI_CLASS(ch)]);
	else
		send_to_char(ch, "You are a level %d %s, you have not selected a multiclass yet.\n\r",
			GET_LEVEL(ch, GET_CLASS(ch)), pc_class_types[GET_CLASS(ch)]);
  /*
        Score additions to display stats
        Salty, 05 JAN 2019
  */
  send_to_char(ch, "You have %s[%s%s%s]HIT %s[%s%s%s]MANA %s[%s%s%s]MOVE\r\n",
          add_commas(GET_HIT(ch)), CCCYN(ch, C_NRM), add_commas(GET_MAX_HIT(ch)), CCNRM(ch, C_NRM),
          add_commas(GET_MANA(ch)), CCCYN(ch, C_NRM), add_commas(GET_MAX_MANA(ch)), CCNRM(ch, C_NRM),
          add_commas(GET_MOVE(ch)), CCCYN(ch, C_NRM), add_commas(GET_MAX_MOVE(ch)), CCNRM(ch, C_NRM));

  send_to_char(ch, "You have [%s%d/%d%s]STR [%s%d%s]INT [%s%d%s]WIS [%s%d%s]DEX [%s%d%s]CON [%s%d%s]LUCK\r\n",
          ...
          CCCYN(ch, C_NRM), GET_LUCK(ch), CCNRM(ch, C_NRM));

  send_to_char(ch, "You have [%s%d%s] DAMROLL and [%s%d%s] HITROLL.\n\r",
          CCCYN(ch, C_NRM), str_app[STRENGTH_APPLY_INDEX(ch)].todam + GET_DAMROLL(ch), CCNRM(ch, C_NRM),
          CCCYN(ch, C_NRM), str_app[STRENGTH_APPLY_INDEX(ch)].tohit + GET_HITROLL(ch), CCNRM(ch, C_NRM));

  send_to_char(ch, "You are carrying %d weight, and the max weight you can carry is %d.\n\r",
  ...
  /*
  	Lets show them how much grinding they did metagaming
  	Salty
  	06 JAN 2019
  */
		send_to_char(ch, "You have saved %s experience points.\n\r",add_commas(GET_EXP(ch)));

  if (GET_REAL_LEVEL(ch) < LVL_MAX_MORTAL)
	{
    send_to_char(ch, "You must carry %s experience points to reach your next level.\r\n",add_commas(level_exp(GET_CLASS(ch), GET_REAL_LEVEL(ch) + 1) - GET_EXP(ch)));
  }


  if (GET_REAL_LEVEL(ch) >= LVL_MAX_MORTAL)
	{
  	send_to_char(ch, "You have spent %s experience points metagaming, you are rank %d.\n\r",add_commas(GET_TOTAL_EXP(ch)), GET_RANK(ch));
	  send_to_char(ch, "You must spend %s experience points metagaming to reach rank %d.\n\r",add_commas(rank_remaining(GET_TOTAL_EXP(ch))), GET_RANK(ch) + 1 );
	}

    send_to_char(ch,"You have earned %s gold coins.\r\n",add_commas(GET_GOLD(ch)));



  send_to_char(ch, "You have earned %s quest points.\r\n", add_commas(GET_QUESTPOINTS(ch)));
  ...
  /*
    Make affects show in score
    Salty
    21 NOV 2019
  */
  //do_affect(ch,"",0,0);

	send_to_char(ch, "Your hitp regen per tick is: %4d, APPLY_HITP_REGEN is %3d.\r\n", hit_gain(ch), GET_HITP_REGEN(ch));
...
ACMD(do_who)
{
  ...
  for (d = descriptor_list; d && !short_list; d = d->next) {
    ...
    if (CAN_SEE(ch, tch) && IS_PLAYING(d)) {
      if (*name_search && str_cmp(GET_NAME(tch), name_search) &&
        ...
        continue;
      if (!CAN_SEE(ch, tch) || GET_REAL_LEVEL(tch) < low || GET_REAL_LEVEL(tch) > high)
        continue;
      ...
      if (showleader && (!GROUP(tch) || GROUP_LEADER(GROUP(tch)) != tch))
        continue;
      for (i = 0; *rank[i].disp != '\n'; i++)
        if (GET_REAL_LEVEL(tch) >= rank[i].min_level && GET_REAL_LEVEL(tch) <= rank[i].max_level)
          rank[i].count++;
    }
  }
  ...
  for (i = 0; *rank[i].disp != '\n'; i++) {
    ...
    for (d = descriptor_list; d; d = d->next) {
      ...
      if ((GET_REAL_LEVEL(tch) < rank[i].min_level || GET_REAL_LEVEL(tch) > rank[i].max_level) && !short_list)
        continue;
      if (!IS_PLAYING(d))
        continue;
      if (*name_search && str_cmp(GET_NAME(tch), name_search) &&
      ...
      if (short_list)
      {
        send_to_char(ch, "%s[%2d %s] %-12.12s%s%s",
          (GET_REAL_LEVEL(tch) >= LVL_IMMORT ? CCYEL(ch, C_SPR) : ""),
          GET_REAL_LEVEL(tch), CLASS_ABBR(tch), GET_NAME(tch),
          CCNRM(ch, C_SPR), ((!(++num_can_see % 4)) ? "\r\n" : ""));
      }
      else
      {
        num_can_see++;
	  static const char *god[] =
				{
    			" IMM ",  /* 0 == 101 */
					" BLD ",	/* 1 == 102 */
					" GOD ",	/* 2 == 103 */
				...
				if (GET_REAL_LEVEL(tch) >= LVL_IMMORT)
				{
					...
					CBGRN(ch, C_SPR),
					god[GET_REAL_LEVEL(tch) - LVL_IMMORT],
					GET_NAME(tch),
          (*GET_TITLE(tch) ? " " : ""),
					GET_TITLE(tch),
          CCNRM(ch, C_SPR));
				...
				{
					if (PLR_FLAGGED(tch, PLR_MULTICLASS))
					{
  					send_to_char(ch, "%s[%2d %s|%s] %s%s%s%s",
						(GET_REAL_LEVEL(tch) >= LVL_IMMORT ? CBYEL(ch, C_SPR) : ""),
						GET_REAL_LEVEL(tch),
      			CLASS_ABBR(tch),
	    			MULTI_CLASS_ABBR(tch),
      			GET_NAME(tch),
      			(*GET_TITLE(tch) ? " " : ""),
						GET_TITLE(tch),
      			CCNRM(ch, C_SPR));
  ...
  if (IS_HAPPYHOUR > 0){
    send_to_char(ch, "It's a Happy Hour! Type \tRhappyhour\tW to see the current bonuses.\tn\r\n");
  }
}
...
ACMD(do_users)
{
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    if (IS_PLAYING(d)) {
      ...
      if (d->original)
	sprintf(classname, "[%2d %s]", GET_REAL_LEVEL(d->original),
		CLASS_ABBR(d->original));
      else
	sprintf(classname, "[%2d %s]", GET_REAL_LEVEL(d->character),
...
ACMD(do_levels)
{
  ...
  size_t len = 0, nlen;
  int i, ret, min_lev=1, max_lev=LVL_MAX_MORTAL, val;
  ...
  if (*arg)
  {
    if (isdigit(*arg))
    {
      ...
      if (ret == 0)
      {
        /* No valid args found */
        min_lev = 1;
        max_lev = LVL_MAX_MORTAL;
      }
      else if (ret == 1)
      {
        /* One arg = range is (num) either side of current level */
        val = min_lev;
        max_lev = MIN(GET_REAL_LEVEL(ch) + val, LVL_MAX_MORTAL);
        min_lev = MAX(GET_REAL_LEVEL(ch) - val, 1);
      }
      else if (ret == 2)
      {
        /* Two args = min-max range limit - just do sanity checks */
        min_lev = MAX(min_lev, 1);
        max_lev = MIN(max_lev + 1, LVL_MAX_MORTAL);
      }
    }
    else
    {
  ...
  for (i = min_lev; i < max_lev + 1; i++)
	{
    nlen = snprintf(buf + len, sizeof(buf) - len, "Level %2d requires:  %12s experience, Default Title:  ", (int)i, add_commas(level_exp(GET_CLASS(ch), i)));
...
/*
  if (len < sizeof(buf) && max_lev == LVL_MAX_MORTAL)
    snprintf(buf + len, sizeof(buf) - len, "[%3d] %15s - Immortality\r\n",
		LVL_IMMORT, add_commas(level_exp(GET_CLASS(ch), LVL_IMMORT)));
*/
...
ACMD(do_consider)
{
  ...
  diff = (GET_REAL_LEVEL(victim) - GET_REAL_LEVEL(ch));
...
ACMD(do_toggle)
{
  ...
  if (*tog_messages[toggle].command == '\n' || tog_messages[toggle].min_level > GET_REAL_LEVEL(ch)) {
...
ACMD(do_commands)
{
  ...
  for (no = 0, cmd_num = 1;
    ...
    if (complete_cmd_info[i].minimum_level < 0 || GET_REAL_LEVEL(ch) < complete_cmd_info[i].minimum_level)
...
ACMD(do_whois)
{
  ...
  if (!(GET_REAL_LEVEL(victim) < LVL_IMMORT) || (GET_REAL_LEVEL(ch) >= GET_REAL_LEVEL(victim))) {
  ...
  if (!got_from_file && victim->desc != NULL && GET_REAL_LEVEL(ch) >= LVL_GOD) {
