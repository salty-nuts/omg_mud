...
bool PrintPermanentAffects(char_data *ch, char *msg)
{
  ...
  if (AFF_FLAGGED(ch, AFF_WAR_DANCE))
    strcat(output, "  War Dance\n\r");
  if (AFF_FLAGGED(ch, AFF_SLOW_DANCE))
    strcat(output, "  Slow Dance\n\r");
  ...
  case SHOW_OBJ_ACTION:
    ...
    case ITEM_KEY:
      if (obj->action_description)
      {
        ...
        send_to_char(ch, "It looks like a key.\r\n");
      break;
    case ITEM_CONTAINER:
      if (obj->action_description)
      {
        ...
        send_to_char(ch, "It looks like a container.\r\n");
      break;
    case ITEM_DRINKCON:
...
static void diag_char_to_char(struct char_data *i, struct char_data *ch)
{
  ...
  if (GET_MAX_HIT(i) > 0)
    percent = (100 * (long)GET_HIT(i)) / (long)GET_MAX_HIT(i);
  else
...
ACMD(do_score)
{
  ...
  send_to_char(ch, "You are a level %d %s\n\r", GET_LEVEL(ch), pc_class_types[GET_CLASS(ch)]);
  ...
  send_to_char(ch, "You have [%s%d%s] DAMROLL, [%s%d%s] HITROLL and [%s%d%s] COMBAT POWER.\n\r",
               ...
               CCCYN(ch, C_NRM), GET_COMBAT_POWER(ch) + GET_RANK(ch), CCNRM(ch, C_NRM));
  ...
  */
  ...
  if (GET_LEVEL(ch) < LVL_MAX_MORTAL)
  {
  ...
  if (GET_LEVEL(ch) >= LVL_MAX_MORTAL)
  {
    send_to_char(ch, "You have spent %s experience points, you are rank %d.\n\r", add_commas(GET_TOTAL_EXP(ch)), GET_RANK(ch));
    send_to_char(ch, "You must spend %s experience points to reach rank %d.\n\r", add_commas(rank_remaining(GET_TOTAL_EXP(ch))), GET_RANK(ch) + 1);
  }
  ...
  for (i = 0; *rank[i].disp != '\n'; i++)
  {
    ...
    for (d = descriptor_list; d; d = d->next)
    {
      ...
      else
      {
        ...
        {
          send_to_char(ch, "%s[%3d %s] %s %s%s%s%s",
                       (GET_LEVEL(tch) >= LVL_IMMORT ? CBYEL(ch, C_SPR) : ""),
                       GET_LEVEL(tch),
                       CLASS_ABBR(tch),
                       rank_name(tch),
                       GET_NAME(tch),
...
ACMD(do_levels)
{
  ...
  if (*arg)
  {
    if (isdigit(*arg))
    {
      ...
      else if (ret == 1)
      {
        ...
        val = min_lev;
        max_lev = MIN(GET_LEVEL(ch) + val, LVL_MAX_MORTAL);
  ...
  {
    total += level_exp(GET_CLASS(ch), i);
    nlen = snprintf(buf + len, sizeof(buf) - len, "Level %2d requires:  %12s experience, %15s total experience\n\r", (int)i,
                    add_commas(level_exp(GET_CLASS(ch), i)), add_commas(total));
