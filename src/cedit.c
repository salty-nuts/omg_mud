...
static void cedit_setup(struct descriptor_data *d)
{
  ...
  OLC_CONFIG(d)->room_nums.recall_room		   = CONFIG_RECALL_ROOM;
...
static void cedit_save_internally(struct descriptor_data *d)
{
  ...
  CONFIG_RECALL_ROOM	= OLC_CONFIG(d)->room_nums.recall_room;
...
int save_config( IDXTYPE nowhere )
{
  ...
  fprintf(fl, "* The virtual number of the recall room\n"
              "recall_room = %d\n\n", CONFIG_RECALL_ROOM);
...
static void cedit_disp_game_play_options(struct descriptor_data *d)
{
  ...
  write_to_output(d, "\r\n\r\n"
        ...
        "%sF%s) Maximum Experience Gain : %s%ld\r\n"
...
static void cedit_disp_room_numbers(struct descriptor_data *d)
{
  ...
  write_to_output(d, "\r\n\r\n"
  	...
  	grn, nrm, cyn, OLC_CONFIG(d)->room_nums.recall_room,
  	grn, nrm
  	);
...
void cedit_parse(struct descriptor_data *d, char *arg)
{
  ...
  switch (OLC_MODE(d)) {
    ...
    case CEDIT_ROOM_NUMBERS_MENU:
      ...
      case '4':
        write_to_output(d, "Enter the vnum for the recall room : ");
        OLC_MODE(d) = CEDIT_RECALL_ROOM;
        return;
   ...
   return;
    ...
    case CEDIT_MAX_EXP_GAIN:
      if (*arg)
        OLC_CONFIG(d)->play.max_exp_gain = atol(arg);
    ...
    case CEDIT_DONATION_ROOM_3:
      if (!*arg) {
        write_to_output(d,
          "That is an invalid choice!\r\n"
          "Enter the vnum for the recall room : ");
      } else if (real_room(atoi(arg)) == NOWHERE) {
        write_to_output(d,
          "That room doesn't exist!\r\n"
          "Enter the vnum for the recall room : ");
   ...
   /* User just pressed return - restore to default */
    ...
    case CEDIT_RECALL_ROOM:
      if (!*arg) {
        ...
        OLC_CONFIG(d)->room_nums.recall_room = atoi(arg);
        cedit_disp_room_numbers(d);
      }
      break;
